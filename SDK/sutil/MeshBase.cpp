  ...
  while(fscanf(file, "%s", buf) != EOF) {
    switch(buf[0]) {
      ...
      case 'u': /* "usemtl <name>" */
        ...
        curr_group = &getOrAddGroup( groupMaterialName( m_merge_mesh_groups ? "MERGED" : curr_group_base_name,
                                                        curr_material_name)   );
        break;
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    switch(buf[0]) {
      ...
      case 'u': /* "usemtl <name>" */
        ...
        curr_group_name = groupMaterialName( m_merge_mesh_groups ?  "MERGED" : curr_group_base_name,
                                             curr_material_name );
